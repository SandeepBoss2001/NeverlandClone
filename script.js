...
function canvas(){
  const canvas = document.querySelector(".canbas");
...
function split(){
  var clutter = ''
  document.querySelector(".NeverLand").textContent.split("").forEach(function(char){
    clutter += `<span>${char}</span>`
  })
  document.querySelector(".NeverLand").innerHTML = clutter
}
split() 

gsap.to(".NeverLand span",{
scrollTrigger:{
  scrub: 1,
  trigger: `.NeverLand span`,
...
function pageSplit(){
  var sutter = ''
  document.querySelector(".paragraph").textContent.split(" ").forEach(function(char){
    sutter += `<span>${char}</span>`
  })
  document.querySelector(".paragraph").innerHTML = sutter
}
pageSplit() 
